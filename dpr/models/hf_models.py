...
def get_bert_entity_biencoder_components(cfg, inference_only: bool = False, **kwargs):
    dropout = cfg.encoder.dropout if hasattr(cfg.encoder, "dropout") else 0.0
    question_encoder = HFBertEntityEncoder.init_encoder(
        cfg.encoder.pretrained_model_cfg,
        projection_dim=cfg.encoder.projection_dim,
        dropout=dropout,
        pretrained=cfg.encoder.pretrained,
...
class HFBertEntityEncoder(BertModel):
    ...
    def forward(
        ...
        if isinstance(representation_token_pos, int):
            ...
            # ent_embeds = ent_embeds.unsqueeze(1)
...
class HFT5Encoder(T5EncoderModel):
    ...
    def init_encoder(
        ...
        if pretrained:
            ...
            return HFT5Encoder(cfg, project_dim=projection_dim)
